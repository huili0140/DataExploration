...
While it's largely a personal choice to be a working mom or not, there might be objective factors that influence the mom's decision. A rough look at the plot of employmennt status showed that overall 65% of the moms were employed and active at work. The proportion of employment steadily increased with the moms' age among the moms younger than 50 years old, suggesting that more moms would like to continue their career track when their kids grew older. Meanwhile, the proportion of being unemployed dropped accordingly with the moms' age. 
...
ggplot(na.omit(wm), aes(x = AGEG, y = log10(WAGP))) + 
  ...
  labs(x = "Age Group", y = "Wage on Log10 Scale", title = "Wages of the Working Moms")
```

### Observation

The instances of working moms were divided into four age groups (16 to 33, 34 to 39, 40 to 45, and 46 to 93) according to their age quantiles. When the wages were plotted according to age groups, a significant increment was shown between the first and the second age groups. The wages of working moms in the second, third and fourth age groups remained relatively on the same level.   

```{r echo=TRUE, message=FALSE, warning=FALSE}
ggplot(na.omit(wm), aes(x = COW, y = log10(WAGP))) + 
  geom_boxplot(aes(fill = COW), alpha = 0.5) + 
  labs(x = "Field of Work", y = "Wageon Log10 Scale", title = "Wages of the Working Moms") +
...
ggplot(na.omit(wm), aes(x = MAR, y = log10(WAGP))) + 
  ...
  labs(x = "Marital Status", y = "Wageon Log10 Scale", title = "Wages of the Working Moms") +
  stat_summary(fun.y=mean, aes(colour = AGEG), geom="point", size = 5) +
  stat_summary(fun.y=mean, aes(group=AGEG, colour = AGEG), geom="line") + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + 
  guides(fill = FALSE)
...
ggplot(na.omit(wm), aes(x = SCHL, y = log10(WAGP))) + 
  ...
  labs(x = "Education", y = "Wageon Log10 Scale", title = "Wages of the Working Moms") +
  stat_summary(fun.y=mean, aes(colour = AGEG), geom="point", size = 5) +
  stat_summary(fun.y=mean, aes(group=AGEG, colour = AGEG), geom="line") + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + 
  guides(fill = FALSE)
...
There was regional differences regarding to the employment rate as well as wages of moms with dependent children. The midwest States (South Dakota, Iowa, Minnesota, Wisconsin, and Nebraska) and the southeast States (Arizona, New Mexico, California, and Utah) represented the extreme cases of highest and lowest rates of moms being employed. The highest wages of working moms were shown in the northeast States (District of Columbia, Maryland, Massachusetts, Connecticut, and New Jersey), which were also the states with highest wages of all employed people. 

# Summary

What can we learn about the moms with dependent children from a glance at the dataset of the 2013 American Community Survey? As a brief description, their ages were bell-shape distributed with a median value of 38 and interquantile range of 13; one quarter of them were single moms either never married or being divorced/separated; they possessed a wide range of education levels from lower than high school to advanced degrees. Moms at older age
