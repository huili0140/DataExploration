...
ggplot(na.omit(wm), aes(x = COW, y = log10(WAGP))) + 
  ...
  labs(x = "Field of Work", y = "Wage on Log10 Scale", title = "Wages vs Field of Work") +
...
ggplot(na.omit(wm), aes(x = MAR, y = log10(WAGP))) + 
  ...
  labs(x = "Marital Status", y = "Wage on Log10 Scale", title = "Wages vs Marital Status") +
  stat_summary(fun.y=mean, aes(colour = AGEG), geom="point", size = 5) +
  stat_summary(fun.y=mean, aes(group=AGEG, colour = AGEG), geom="line") + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + 
  guides(fill = FALSE)
...
ggplot(na.omit(wm), aes(x = SCHL, y = log10(WAGP))) + 
  ...
  labs(x = "Education", y = "Wage on Log10 Scale", title = "Wages vs Education") +
  stat_summary(fun.y=mean, aes(colour = AGEG), geom="point", size = 5) +
  stat_summary(fun.y=mean, aes(group=AGEG, colour = AGEG), geom="line") + 
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + 
  guides(fill = FALSE)
...
#########################
```{r echo=TRUE, message=FALSE, warning=FALSE}
subCOW <- subset(wm, AGEP == 39 & COW == "Private profit" & MAR == "Married" & SCHL == "Bachelor's degree")
ggplot(subPAOC, aes(x = PAOC, y = log10(WAGP))) + 
  geom_boxplot(aes(fill = PAOC), alpha = 0.5) + 
  labs(x = "Presence of Children", y = "Wage on Log10 Scale") +
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + 
  guides(fill = FALSE)

subCOW <- subset(wm, AGEP == 39 & MAR == "Married" & SCHL == "Bachelor's degree")
ggplot(subCOW, aes(x = COW, y = log10(WAGP))) + 
  geom_boxplot(aes(fill = COW), alpha = 0.5) + 
  labs(x = "Presence of Children", y = "Wage on Log10 Scale") +
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + 
  guides(fill = FALSE)

subMAR <- subset(wm, AGEP == 39 & COW == "Private profit" & SCHL == "Bachelor's degree")
ggplot(subMAR, aes(x = MAR, y = log10(WAGP))) + 
  geom_jitter(aes(colour = MAR), alpha = 0.5) + 
  labs(x = "Presence of Children", y = "Wage on Log10 Scale") +
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + 
  guides(fill = FALSE)

subSCHL <- subset(wm, AGEP == 39 & COW == "Private profit" & MAR == "Married")
ggplot(subSCHL, aes(x = SCHL, y = log10(WAGP))) + 
  geom_jitter(aes(colour = SCHL), alpha = 0.5) + 
  labs(x = "Presence of Children", y = "Wage on Log10 Scale") +
  theme(axis.text.x = element_text(angle = 30, hjust = 1)) + 
  guides(fill = FALSE)
